...
const JobRoleModal: React.FC<JobRoleModalProps> = ({ jobRole, onClose, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = jobRole
        ? await api.put(`/job-roles/${jobRole._id}`, formData)
        : await api.post('/job-roles', formData);

      onSave(response.data);
      toast.success(`Job role ${jobRole ? 'updated' : 'created'} successfully`);
    } catch (error: any) {
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-white/20">
          <h2 className="text-xl font-bold text-white">
            {jobRole ? 'Edit Job Role' : 'Add New Job Role'}
          </h2>
          <button
            onClick={onClose}
