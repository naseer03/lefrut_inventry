...
interface DocumentTypeModalProps {
  ...
  onClose: () => void;
  onSave: (documentType: DocumentType) => void;
}

const DocumentTypeModal: React.FC<DocumentTypeModalProps> = ({ documentType, onClose, onSave }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = documentType
        ? await api.put(`/document-types/${documentType._id}`, formData)
        : await api.post('/document-types', formData);

      onSave(response.data);
      toast.success(`Document type ${documentType ? 'updated' : 'created'} successfully`);
    } catch (error: any) {
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-white/20">
          <h2 className="text-xl font-bold text-white">
            {documentType ? 'Edit Document Type' : 'Add New Document Type'}
          </h2>
          <button
            onClick={onClose}
